...
> [!TIP]
> **Production Deployment (500.000 Baris)**
> Untuk mengakomodasi performa tinggi dari **Denormalisasi** secara aman di Production (mempertimbangkan batas 1GB Aiven *free-tier* dan *transaction log* saat *backfill*), jumlah *seeder* data awal di Aiven dibatasi secara konservatif di angka **500.000 baris**. Dengan limitasi organik ini, skema denormalisasi berhasil di-*deploy* ke Production secara utuh tanpa risiko *crash out-of-storage*.
...
| TS-13 | Export seluruh dataset (500 Ribu) | ✅ Streaming tanpa OOM & Timeout, tidak ada crash |
...
- **Online (Aiven Free Tier):** Mengingat Aiven MySQL hanya memberikan kapasitas gratis maksimal 1 GB, fitur **Denormalisasi Sorting** yang menambah duplikasi data dan *transaction log* menuntut penyesuaian volume data yang sangat konservatif. Pengujian *live* telah menetapkan sistem ini **stabil dan sangat aman di angka 500.000 baris data secara online**.

> **Catatan Penting Deploy:** 
> Fitur *Denormalisasi Sorting* dilindungi oleh variabel lingkungan. Di *live server* (Railway), konfigurasi `USE_DENORMALIZATION=true` diaktifkan secara bawaan melalui `railway.toml`. Ini memastikan mekanisme sorting otomatis menggunakan *Direct Index Scan* berbasis denormalisasi yang sangat cepat (41ms).

**Command yang dijalankan:**
* **Di Localhost (untuk 5 Juta Data):** 
  `php artisan app:seed-enrollments`
* **Di Produksi Online (Telah Disesuaikan untuk Denormalisasi: 500.000 Data):** 
  `php artisan app:seed-enrollments --count=500000`
